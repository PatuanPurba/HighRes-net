...
from DeepNetworks.HRNet_New import ConvertorNoPool
...
def trainAndGetBestModel(convertor, fusion_model:nn.modules, regis_model, optimizer, dataloaders, baseline_cpsnrs, config):
    """
    ...
    # Additional layer for fusion model
    ...
    for epoch in tqdm(range(1, num_epochs + 1)):
        ...
        if best_score > val_score:
            torch.save(convertor.state_dict(),
                       os.path.join(checkpoint_dir_run, 'fusion_model_best.pth'))
...
def main(config):
    ...
    convertor_model = ConvertorNoPool(config["network"])

    checkpoint_dir = config["paths"]["checkpoint_dir"]
    run_subfolder = 'batch_20_views_32_min_32_beta_50.0_time_2025-11-17-10-26-29-900576'
    ...
    fusion_model.load_state_dict(torch.load(checkpoint_file))

    regis_model = ShiftNet()


    optimizer = optim.Adam(list(convertor_model.parameters()),
                           lr=config["training"]["lr"])  # optim

    # For DataLoader
    # ESA dataset

    # Workflow: Get the image directories, store as a list, then split into train and test
    ...
    trainAndGetBestModel(convertor_model, fusion_model, regis_model, optimizer, dataloaders, baseline_cpsnrs, config)
